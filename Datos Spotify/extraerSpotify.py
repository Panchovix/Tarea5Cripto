...
imap.login('mail', 'contraseña')
imap.select('Inbox')
_, x = imap.search(None,'FROM', 'no-reply@spotify.com')
...
def getMsgID(x):
    ...
    for num in x[0].split():
        ...
        print(datito)
        a += 1
        with open('MsgIDsSpotify.txt', 'a') as f: #se crea el archivo de texto con el data set
...
def getSecondLastReceived(x):
    ...
    for num in x[0].split():
        ...
        a += 1
        with open('SecondLastReceivedSpotify.txt', 'a') as f: #se crea el archivo de texto con el data set
            with redirect_stdout(f):
...
def getFirstReceived(x):
    ...
    for num in x[0].split():
        ...
        a += 1
        with open('FirstReceivedSpotify.txt', 'a') as f: #se crea el archivo de texto con el data set
            with redirect_stdout(f):
...
def getUTC(x):
    ...
    for num in x[0].split():
        ...
        d33 = str(d3[-15:])
        a += 1
        with open('UTCsSpotify.txt', 'a') as f: #se crea el archivo de texto con el data set
            with redirect_stdout(f):
                print (d33)
...
def getFrom(x):
    ...
    for num in x[0].split():
        ...
        print(datito4)
        a += 1
        with open('MailsSpotify.txt', 'a') as f: #se crea el archivo de texto con el data set
            with redirect_stdout(f):
                print(datito4)           
